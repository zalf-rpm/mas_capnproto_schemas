...
$Go.package("registry");
$Go.import("github.com/zalf-rpm/mas_capnproto_schemas/gen/go/registry");

using Identifiable = import "/common/common.capnp".Identifiable;
using IdInformation = import "/common/common.capnp".IdInformation;
using Persistence = import "/persistence/persistence.capnp";
...
interface Registry extends(Identifiable) {
  ...
  struct Entry {
    categoryId  @0 :Text;           # category id
    ref         @1 :Identifiable;   # reference to remote Identifiable object
    name        @2 :Text;           # mostly display name
    id          @3 :Text;           # id of Identifiable, to be able to manage entries without access to ref
  }
